...
class HomeController extends GetxController {
  ...
  @override
  void onReady() {
    HomeTopService.to.homePagechanged(0);
  }
  ...
  Route? onGenerateRoute(RouteSettings settings) {
    if (settings.name == Routes.FOUND) {
      ...
      return GetPageRoute(
          settings: settings,
          page: () => const KeepAliveWrapper(child: PodcastPage()),
          binding: PodcastBinding(),
          transition: Transition.noTransition);
    } else if (settings.name == Routes.MINE) {
      return GetPageRoute(
          settings: settings,
          page: () => const KeepAliveWrapper(child: MinePage()),
          binding: MineBinding(),
          transition: Transition.noTransition);
    } else if (settings.name == Routes.K_SONG) {
      return GetPageRoute(
          settings: settings,
          page: () => const KeepAliveWrapper(child: KSongPage()),
          binding: KSongBinding(),
          transition: Transition.noTransition);
    } else if (settings.name == Routes.CLOUD_VILLAGE) {
      return GetPageRoute(
          settings: settings,
          page: () => const KeepAliveWrapper(child: CloudVillagePage()),
